...
GlueDatabaseName='security_analytics' #Name of the Glue database to create, which will contain all security analytics tables created by this template (**cannot contain hyphen**)

# CloudTrail Table Parameters
CloudTrailTableEnabled=True #Create and enable a table for CloudTrail
...
VPCFlowTableEnabled=True #'Create and enable a table for VPC Flow Logs
...
VPCFlowAccountEnum='<account_num_1>,<account_num_2>,...' #Account(s) to include in the VPC flow log table in a comma separated list with NO SPACES (example: "0123456789,0123456788,0123456777"); note that all accounts must be logging to the same source, with contents in {ParamVPCFlowSource}/AWSLogs/{account_number}/vpcflowlogs/'
VPCFlowRegionEnum='us-east-2,us-east-1,us-west-1,us-west-2,af-south-1,ap-east-1,ap-south-1,ap-northeast-3,ap-northeast-2,ap-southeast-1,ap-southeast-2,ap-northeast-1,ca-central-1,cn-north-1,cn-northwest-1,eu-central-1,eu-west-1,eu-west-2,eu-south-1,eu-west-3,eu-north-1,me-south-1,sa-east-1' #Regions to include in the VPC flow log table in a comma separated list with NO SPACES; Include all regions for full coverage even if there are no logs currently in that region

# DNS Resolver Log Table Parameters
DNSResolverTableEnabled=True #Create and enable a table for Route53 DNS Resolver Logs'
...
DNSResolverAccountEnum='<account_num_1>,<account_num_2>,...' #Account(s) to include in the Route53 DNS Resolver log table in a comma separated list with NO SPACES (example: "0123456789,0123456788,0123456777"); note that all accounts must be logging to the same source, with contents in {ParamVPCFlowSource}/AWSLogs/{account_number}/vpcdnsquerylogs/
...
ALBTableEnabled=True #Create and enable a table for ALB logs
ALBRegions='us-east-1,us-east-2,...' #Region(s) where ALB logs are stored
ALBTableName='alb' #'Name of the ALB Glue table to create
ParamALBSource='s3://<bucket>/<prefix>/AWSLogs/' #S3 bucket path where logs are stored (including any custom prefix) with NO SPACES (must end with /AWSLogs/) and be located in the region specified in ParamALBRegion
...
ALBAccountEnum='<account_num_1>,<account_num_2>,...'#Account(s) to include in the ALB log table in a comma separated list with NO SPACES (example: "0123456789,0123456788,0123456777"); note that all accounts must be logging to the same source, with contents in {ParamALBSource}/AWSLogs/{account_number}/ALB
